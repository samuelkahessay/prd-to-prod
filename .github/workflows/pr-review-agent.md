...
6. **Read the authoritative PRD source**.
   ...
   - Build a checklist of the in-scope normative requirements (the exact, non-negotiable details from the spec: endpoint paths, HTTP status codes, field names, counts, thresholds, exact UI text): exact endpoint paths, query params, HTTP methods, status codes, payload fields, enum values, counts, thresholds, exact UI text/headings, required ordering, required tests, and explicit `must not` constraints.
